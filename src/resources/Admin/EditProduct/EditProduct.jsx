...
function EditProduct() {
    ...
    const fetchDetailProduct = () => {
        axios.get("http://localhost:3000/product/detail/" + id)
            .then(res => {
                ...
                setidCategori(res.data.ID_Categori)
                setidBrand(res.data.ID_Brand)
                setContentMarkdown(res.data.contentMarkdown)
                setContentHTML(res.data.contentHTML)
                setShowImage(`http://localhost:3000/${res.data.imageProduct}`)
            })
    ...
    const uploadService = () => {
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            console.log("Data sumbit", formData)
            axios.put("http://localhost:3000/product/detail/" + id, formData)
                .then(res => {
